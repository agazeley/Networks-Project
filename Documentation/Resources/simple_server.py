...
class simple_server:
    ...
    def create_http_header(self,code,location = '/'):
        # depending on response code we will send a different response
        header = ""
        if code == 200:
            header = 'HTTP/1.1 200 OK\n'
        elif code == 303:
            header = 'HTTP/1.1 303 Redirect\n Location: ' + self.host + str(location) + ' \n'
        elif code == 400:
            header = 'HTTP/1.1 400 Bad Request\n'
        elif code == 404:
            header = 'HTTP/1.1 404 Not Found \n'

        # Create datetime for header, server tag, and connection tag
        datetime = time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime())
        header += "Date: " + str(datetime) + "\n"
        header += "Server: Andrew's Simple Python Server\n"
